# Operator Data Conflicts

Source snapshot: repository authored sources. Generated deterministically without a runtime date.

Coverage: **29 authored reviews**, **4 homepage cards**, **10 comparison rows**, and **46 relevant hub facts**.

No conflict below is resolved by this audit. Values remain exactly as authored pending source review.

`src/data/operators.ts` records these conflicts as `unresolved`; canonical selectors and Review schema omit them. Verified canonical values retain field-level provenance, while affiliate restrictions and schema identity remain in their separate data modules.

| Operator | Field | Exact source values | Status |
|---|---|---|---|
| acebet | editorial score | `index.html` = `4.6/5`<br>`reviews/acebet.html` = `88/100 (4.4/5)`<br>`reviews/acebet.html JSON-LD Review.reviewRating` = `4.5/5` | UNRESOLVED |
| big-pirate | editorial score | `index.html` = `4.7/5`<br>`reviews/big-pirate.html` = `79/100 (3.95/5)`<br>`reviews/big-pirate.html JSON-LD Review.reviewRating` = `4.1/5` | UNRESOLVED |
| card-crush | editorial score | `index.html` = `4.2/5`<br>`reviews/card-crush.html` = `82/100 (4.1/5)`<br>`reviews/card-crush.html JSON-LD Review.reviewRating` = `4.2/5` | UNRESOLVED |
| casino-click | editorial score | `index.html` = `4.7/5`<br>`reviews/casino-click.html` = `72/100 (3.6/5)`<br>`reviews/casino-click.html JSON-LD Review.reviewRating` = `3.8/5` | UNRESOLVED |
| crown-coins | editorial score | `index.html` = `4.8/5`<br>`reviews/crown-coins.html` = `91/100 (4.55/5)`<br>`reviews/crown-coins.html JSON-LD Review.reviewRating` = `4.6/5` | UNRESOLVED |
| crown-coins | welcome offer | `index.html` = `100,000 Crown Coins + 2 SC No Deposit`<br>`src/content/comparisons/sweepstakes-casinos.mdx` = `100,000 CC + 2 SC`<br>`src/routes/bonuses/no-deposit/index.astro` = `100,000 CC + 2 SC` | UNRESOLVED |
| dexyplay | editorial score | `index.html` = `4.8/5`<br>`reviews/dexyplay.html` = `87/100 (4.35/5)`<br>`reviews/dexyplay.html JSON-LD Review.reviewRating` = `4.5/5` | UNRESOLVED |
| freespin | editorial score | `index.html` = `4.9/5`<br>`reviews/freespin.html` = `82/100 (4.1/5)`<br>`reviews/freespin.html JSON-LD Review.reviewRating` = `4.3/5` | UNRESOLVED |
| hello-millions | editorial score | `index.html` = `4.6/5`<br>`reviews/hello-millions.html` = `85/100 (4.25/5)`<br>`reviews/hello-millions.html JSON-LD Review.reviewRating` = `4.2/5` | UNRESOLVED |
| hello-millions | welcome offer | `index.html` = `7,500 GC + 2.5 SC Promo Code`<br>`src/content/comparisons/sweepstakes-casinos.mdx` = `15,000 GC + 2.5 SC`<br>`src/routes/bonuses/no-deposit/index.astro` = `15,000 GC + 2.5 SC` | UNRESOLVED |
| high5 | editorial score | `index.html` = `4.9/5`<br>`reviews/high5.html` = `88/100 (4.4/5)`<br>`reviews/high5.html JSON-LD Review.reviewRating` = `4.3/5` | UNRESOLVED |
| jackpot-go | editorial score | `index.html` = `4.5/5`<br>`reviews/jackpot-go.html` = `85/100 (4.25/5)`<br>`reviews/jackpot-go.html JSON-LD Review.reviewRating` = `4.4/5` | UNRESOLVED |
| jackpota | editorial score | `index.html` = `4.7/5`<br>`reviews/jackpota.html` = `86/100 (4.3/5)`<br>`reviews/jackpota.html JSON-LD Review.reviewRating` = `4.3/5` | UNRESOLVED |
| lucky-bunny | editorial score | `index.html` = `4.9/5`<br>`reviews/lucky-bunny.html` = `74/100 (3.7/5)`<br>`reviews/lucky-bunny.html JSON-LD Review.reviewRating` = `3.9/5` | UNRESOLVED |
| mcluck | editorial score | `index.html` = `4.5/5`<br>`reviews/mcluck.html` = `88/100 (4.4/5)`<br>`reviews/mcluck.html JSON-LD Review.reviewRating` = `4.5/5` | UNRESOLVED |
| mega-bonanza | editorial score | `index.html` = `4.5/5`<br>`reviews/mega-bonanza.html` = `82/100 (4.1/5)`<br>`reviews/mega-bonanza.html JSON-LD Review.reviewRating` = `4/5` | UNRESOLVED |
| pulsz | editorial score | `index.html` = `4.5/5`<br>`reviews/pulsz.html` = `88/100 (4.4/5)`<br>`reviews/pulsz.html JSON-LD Review.reviewRating` = `4.5/5` | UNRESOLVED |
| rolla | editorial score | `index.html` = `5/5`<br>`reviews/rolla.html` = `92/100 (4.6/5)`<br>`reviews/rolla.html JSON-LD Review.reviewRating` = `4.7/5` | UNRESOLVED |
| spinblitz | editorial score | `index.html` = `4.6/5`<br>`reviews/spinblitz.html` = `87/100 (4.35/5)`<br>`reviews/spinblitz.html JSON-LD Review.reviewRating` = `4.4/5` | UNRESOLVED |
| spinblitz | welcome offer | `index.html` = `7,500 GC + 2.5 SC Promo Code`<br>`src/routes/bonuses/no-deposit/index.astro` = `7,500 GC + 2.5 SC` | UNRESOLVED |
| spinfinite | editorial score | `index.html` = `4.5/5`<br>`reviews/spinfinite.html` = `80/100 (4/5)`<br>`reviews/spinfinite.html JSON-LD Review.reviewRating` = `4.1/5` | UNRESOLVED |
| splash-coins | editorial score | `index.html` = `4.9/5`<br>`reviews/splash-coins.html` = `83/100 (4.15/5)`<br>`reviews/splash-coins.html JSON-LD Review.reviewRating` = `4.3/5` | UNRESOLVED |
| spree | editorial score | `index.html` = `4.6/5`<br>`reviews/spree.html` = `83/100 (4.15/5)`<br>`reviews/spree.html JSON-LD Review.reviewRating` = `4/5` | UNRESOLVED |
| spree | welcome offer | `index.html` = `25,000 GC + 2.5 SC Instant Reg`<br>`src/content/comparisons/sweepstakes-casinos.mdx` = `25,000 GC + 2.5 SC`<br>`src/routes/bonuses/no-deposit/index.astro` = `25,000 GC + 2.5 SC` | UNRESOLVED |
| sweepico | editorial score | `index.html` = `4.6/5`<br>`reviews/sweepico.html` = `85/100 (4.25/5)`<br>`reviews/sweepico.html JSON-LD Review.reviewRating` = `4.4/5` | UNRESOLVED |
| sweet-sweeps | editorial score | `index.html` = `4.7/5`<br>`reviews/sweet-sweeps.html` = `90/100 (4.5/5)`<br>`reviews/sweet-sweeps.html JSON-LD Review.reviewRating` = `4.5/5` | UNRESOLVED |
| thrillzz | editorial score | `index.html` = `4.3/5`<br>`reviews/thrillzz.html` = `85/100 (4.25/5)`<br>`reviews/thrillzz.html JSON-LD Review.reviewRating` = `4.3/5` | UNRESOLVED |
| wow-vegas | editorial score | `index.html` = `4.8/5`<br>`reviews/wow-vegas.html` = `91/100 (4.55/5)`<br>`reviews/wow-vegas.html JSON-LD Review.reviewRating` = `4.5/5` | UNRESOLVED |
| zula | editorial score | `index.html` = `4.4/5`<br>`reviews/zula.html` = `87/100 (4.35/5)`<br>`reviews/zula.html JSON-LD Review.reviewRating` = `4.4/5` | UNRESOLVED |

## Review inventory

- `reviews/acebet.html` — Acebet Casino Review — Redemption, Bonus & Payouts (2026)
- `reviews/american-luck.html` — American Luck Review 2026: Is It Legit? \| Sweepstakes Wiz
- `reviews/big-pirate.html` — Big Pirate Casino Review (2026): Legit, Payout Speed & Bonus
- `reviews/card-crush.html` — Card Crush Review (2026): Legit, Payouts — CA & NY Only
- `reviews/casino-click.html` — Casino Click Review (2026): Legit, Payout Speed & Daily 5 SC Bonus
- `reviews/crown-coins.html` — Crown Coins Review — Redemption, Bonus & Trustpilot (2026)
- `reviews/dexyplay.html` — DexyPlay Casino Review (2026): Legit, PayPal Payouts & Bonus
- `reviews/freespin.html` — FreeSpin Casino Review — Redemptions & Promo Codes (2026)
- `reviews/hello-millions.html` — Hello Millions Review (2026): Legit, Payouts & Phone Support
- `reviews/high5.html` — High 5 Casino Review 2026 — Redemption & Diamond Currency
- `reviews/jackpot-go.html` — Jackpot Go Casino Review 2026 — Payouts & Promo Codes
- `reviews/jackpota.html` — Jackpota Casino Review (2026): Legit, Payouts & Jackpots
- `reviews/legendz.html` — Legendz Casino Review (2026): Legit, Payout Speed & Sportsbook
- `reviews/lucky-bunny.html` — Lucky Bunny Casino Review 2026 — Redemptions, Bonus Codes
- `reviews/mcluck.html` — McLuck Review 2026 — Bonus, McJackpots, Payouts & Apps
- `reviews/mega-bonanza.html` — Mega Bonanza Review 2026 — Redemption & Bonuses
- `reviews/playfame.html` — PlayFame Casino Review (2026): Legit, Payouts, iOS App & Bonus
- `reviews/pulsz.html` — Pulsz Review 2026 — Bonus, Apps, Payouts & Trustpilot
- `reviews/rolla.html` — Rolla Casino Review — Redemption & Payout Speed Explained (2026)
- `reviews/roxymoxy.html` — RoxyMoxy Casino Review (2026): Legit, Payouts & Restricted States
- `reviews/spinblitz.html` — SpinBlitz Casino Review 2026 — Redemption & Jackpots
- `reviews/spinfinite.html` — Spinfinite Casino Review 2026 - Is it Legit?
- `reviews/splash-coins.html` — Splash Coins Casino Review 2026 — Redemption, Bonus Explained
- `reviews/spree.html` — Spree Casino Review 2026 — Redemption Speed & Jackpots
- `reviews/sweepico.html` — Sweepico Casino Review 2026 — Redemption & Bonuses
- `reviews/sweet-sweeps.html` — Sweet Sweeps Casino Review 2026 — Is it Legit?
- `reviews/thrillzz.html` — Thrillzz Review 2026 — Social Sportsbook, Bonus & Payouts
- `reviews/wow-vegas.html` — WOW Vegas Casino Review 2026 — Redemption Speed & Bonus
- `reviews/zula.html` — Zula Casino Review (2026): Legit, Payouts & 29K Trustpilot Reviews

## Homepage inventory

- `playfame` — `86/100`; offer `7,500 GC + 2.5 SC`
- `legendz` — `84/100`; offer `500 GC + 3 SC`
- `roxymoxy` — `80/100`; offer `50,000 GC + 2.5 SC`
- `american-luck` — `72/100`; offer `—`

## Relevant hub inventory

- `src/routes/bonuses/no-deposit/index.astro` — `casino-click` minimum redemption: `100 SC (cash)`
- `src/routes/bonuses/no-deposit/index.astro` — `casino-click` welcome offer: `100,000 GC + 2 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `crown-coins` minimum redemption: `100 SC (cash)`
- `src/routes/bonuses/no-deposit/index.astro` — `crown-coins` welcome offer: `100,000 CC + 2 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `hello-millions` minimum redemption: `10 SC (gift cards)`
- `src/routes/bonuses/no-deposit/index.astro` — `hello-millions` welcome offer: `15,000 GC + 2.5 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `legendz` minimum redemption: `50 SC (gift cards); 100 SC (cash)`
- `src/routes/bonuses/no-deposit/index.astro` — `legendz` welcome offer: `500 GC + 3 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `mcluck` minimum redemption: `10 SC (gift cards)`
- `src/routes/bonuses/no-deposit/index.astro` — `mcluck` welcome offer: `7,500 GC + 2.5 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `playfame` minimum redemption: `10 SC (gift cards)`
- `src/routes/bonuses/no-deposit/index.astro` — `playfame` welcome offer: `7,500 GC + 2.5 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `pulsz` minimum redemption: `10 SC (gift cards)`
- `src/routes/bonuses/no-deposit/index.astro` — `pulsz` welcome offer: `5,000 GC + 2.3 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `roxymoxy` minimum redemption: `100 SC (cash)`
- `src/routes/bonuses/no-deposit/index.astro` — `roxymoxy` welcome offer: `50,000 GC + 2.5 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `spinblitz` welcome offer: `7,500 GC + 2.5 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `spree` minimum redemption: `10 SC (gift cards)`
- `src/routes/bonuses/no-deposit/index.astro` — `spree` welcome offer: `25,000 GC + 2.5 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `thrillzz` minimum redemption: `50 SC (cash)`
- `src/routes/bonuses/no-deposit/index.astro` — `thrillzz` welcome offer: `3,000 GC + 3 SC`
- `src/routes/bonuses/no-deposit/index.astro` — `zula` minimum redemption: `50 SC (cash)`
- `src/routes/bonuses/no-deposit/index.astro` — `zula` welcome offer: `Up to 120,000 GC + 10 SC`
- `src/routes/new/index.astro` — `american-luck` curated hub note: `New US operator (SGSE LLC, Delaware). No-purchase model, 1,500+ games, cash prizes via ACH from a low 50 SC minimum. Cash-out is ACH-only and the track record is short.`
- `src/routes/new/index.astro` — `big-pirate` curated hub note: `Newer non-partner brand with a three-currency system, a Claw Machine feature and 1,500+ games (editorial, info-only review).`
- `src/routes/new/index.astro` — `card-crush` curated hub note: `Card-battle RPG twist on the sweeps model using Mystery Coins (1 MC = $1). Cash from 75 MC, gift cards from 10 MC. Available only in CA & NY; 21+.`
- `src/routes/new/index.astro` — `jackpota` curated hub note: `Gift cards reported in ~2 days from 10 SC; 80,000 GC + 40 SC first-purchase pack; 1,600+ games incl. live dealer (info-only review).`
- `src/routes/new/index.astro` — `legendz` curated hub note: `Social sportsbook + casino. No-code 500 GC + 3 SC; Prizeout gift cards from 50 SC, Skrill/bank cash from 100 SC.`
- `src/routes/new/index.astro` — `playfame` curated hub note: `PlayFame Operations Ltd. 7,500 GC + 2.5 SC welcome, 10 SC gift cards and bank-transfer cash, plus a dedicated iOS app.`
- `src/routes/new/index.astro` — `roxymoxy` curated hub note: `US operator (Rainforest LTD, Delaware). No-code 50,000 GC + 2.5 SC welcome; bank-transfer redemptions from 100 SC; 40+ slots, browser-only.`
- `src/routes/new/index.astro` — `spinfinite` curated hub note: `Subscription-style Power Pass and instant gift cards from 10 SC, with a 3 SC mail-in AMOE and an Android app (info-only review).`
- `src/routes/new/index.astro` — `spree` curated hub note: `Play Spree Ltd. 10 SC gift-card redemptions, SpreePotz jackpots and Sit-and-Spin, 800+ games.`
- `src/routes/new/index.astro` — `thrillzz` curated hub note: `Social sportsbook (Thrillzz Inc., TN). 50 SC redemptions via bank/PayPal/Skrill in 1–3 days, with native iOS/Android apps.`
- `src/routes/state-legality/index.astro` — `card-crush` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `casino-click` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `crown-coins` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `hello-millions` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `legendz` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `mcluck` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `playfame` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `pulsz` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `roxymoxy` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `spinblitz` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `spree` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `thrillzz` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
- `src/routes/state-legality/index.astro` — `zula` operator availability authority: `AFFILIATE_PARTNERS + tracker status + site CTA policy`
